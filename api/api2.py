...
def test(model: Model, core: Core, data_loader: torch.utils.data.DataLoader, nc, validator, num_samples: int = None):
    ...
    for batch_i, batch in enumerate(tqdm(data_loader, total=num_samples)):
        ...
        if num_outputs == 1:
            ...
            preds = [torch.from_numpy(results[compiled_model.output(0)]), torch.from_numpy(
                results[compiled_model.output(1)])]
...
def print_stats(stats: np.ndarray, total_images: int, total_objects: int):
    ...
    """
    print("Boxes:")
    mp, mr, map50, mean_ap = stats['metrics/precision(B)'], stats['metrics/recall(B)'], stats[
        'metrics/mAP50(B)'], stats['metrics/mAP50-95(B)']
    # Print results
...
def draw_results(results: Dict, source_image: np.ndarray, label_map: Dict):
    ...
    return source_image, count,boxes
...
def detect(image: np.ndarray, model: Model, nc):
    ...
    detections = postprocess(
        nc=nc, pred_boxes=boxes, input_hw=input_hw, orig_img=image, pred_masks=masks)
    return detections
